"""SKY130A boundary for ShapeIC's generic Magic orchestration."""
...
class Sky130ATechnology:
    ...
    def normalize_macro_pex(
        ...
        if set(bulk_ports) != expected:
            raise ValueError(
                f"SKY130A macro bulk bindings must define {sorted(expected)}, "
        ...
        for line in _logical_lines(text):
            ...
            polarity = _model_polarity(model)
            if polarity is None:
                continue
        ...
        if not replacements:
            raise ValueError(
                "extracted macro contains no recognized SKY130A MOS bulk nodes"
            )
        output = []
    ...
    def normalize_mos_device(
        ...
        if _model_polarity(_device_model(fields)) is None:
            raise ValueError("unrecognized extracted SKY130A MOS instance")
    ...
    def validate_primitive_pex(
        ...
        for line in lines:
            ...
            if kind == "r" and len(fields) >= 4:
                ...
                continue
            model = _device_model(fields)
            model_polarity = _model_polarity(model)
            if model_polarity is None:
                continue
        ...
        if not devices:
            raise ValueError("primitive PEX contains no recognized SKY130A MOS devices")
...
def _bulk_index(fields: list[str]) -> int:
    if _device_model(fields) is None or len(fields) < 6:
        raise ValueError("an extracted SKY130A MOS instance requires four terminals")
    return 4
